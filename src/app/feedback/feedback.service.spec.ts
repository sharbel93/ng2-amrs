...
describe('FeedBackService', () => {
  ...
  it('should be defined', inject([FeedBackService], (s: FeedBackService) => {
    expect(s).toBeDefined();
  }));
  it('should call the right url when postFeedback is called', inject(
    [FeedBackService, AppSettingsService, MockBackend, Http],
    (feedbackService, appSettingsService, backend, http) => {
      let samplePayload = { phone: '070000000', message: 'message' };
      backend.connections.subscribe((connection: MockConnection) => {
        let url = appSettingsService.getEtlServer() + '/user-feedback';
